...
export const MaturadorTab = () => {
  ...
  const handleStartMaturador = () => {
    if (maturadorEngine.chipPairs.length === 0) {
      toast({ title: "Erro", description: "Configure pelo menos uma dupla.", variant: "destructive" });
      return;
    }

    const activePairs = maturadorEngine.chipPairs.filter(pair => pair.isActive);
    if (activePairs.length === 0) {
      toast({ title: "Erro", description: "Ative ao menos uma dupla.", variant: "destructive" });
      return;
    }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Switch checked={maturadorEngine.isRunning} onCheckedChange={handleStartMaturador}/>
          ...
          >
            {maturadorEngine.isRunning ? (
              <><Square className="w-4 h-4 mr-2" />Parar Maturador</>
            ) : (
              <><Play className="w-4 h-4 mr-2" />Iniciar Maturador</>
            )}
          </Button>
        </div>
      </div>
